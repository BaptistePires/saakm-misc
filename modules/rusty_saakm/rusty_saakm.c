...
static void tuner_step(void)
{
	...
	//current cpustat
	u64 new_user, new_nice, new_system, new_idle, new_iowait, new_irq,
		new_softirq, new_steal;
	new_user = new_nice = new_system = new_idle = new_iowait = new_irq =
		new_softirq = new_steal = 0;

	//util cpustat
	bool enable_direct, enable_kick;
	enable_direct = enable_kick = false;

	u64 util_user, util_nice, util_system, util_idle, util_iowait, util_irq,
		util_softirq, util_steal;
	util_user = util_nice = util_system = util_idle = util_iowait =
		util_irq = util_softirq = util_steal = 0;
	...
	for_each_possible_cpu(cpu) {
		...
		for_each_cpu(dom_cpu, &l->cores) { //TODO check if numa level ?
			...
			//the new stats
			new_user = div_u64(cpustat[CPUTIME_USER],
					   NSEC_PER_SEC / USER_HZ);
			new_nice = div_u64(cpustat[CPUTIME_NICE],
					   NSEC_PER_SEC / USER_HZ);
			new_system = div_u64(cpustat[CPUTIME_SYSTEM],
					     NSEC_PER_SEC / USER_HZ);
			new_idle = div_u64(cpustat[CPUTIME_IDLE],
					   NSEC_PER_SEC / USER_HZ);
			new_iowait = div_u64(cpustat[CPUTIME_IOWAIT],
					     NSEC_PER_SEC / USER_HZ);
			new_irq = div_u64(cpustat[CPUTIME_IRQ],
					  NSEC_PER_SEC / USER_HZ);
			new_softirq = div_u64(cpustat[CPUTIME_SOFTIRQ],
					      NSEC_PER_SEC / USER_HZ);
			new_steal = div_u64(cpustat[CPUTIME_STEAL],
					    NSEC_PER_SEC / USER_HZ);

			//<=> sub_or_zero between cur and prev, that's a function in tuner.rs
			util_user = (new_user - c->user) < 0 ?
					    0 :
					    (new_user - c->user);
			util_nice = (new_nice - c->nice) < 0 ?
					    0 :
					    (new_nice - c->nice);
			util_system = (new_system - c->system) < 0 ?
					      0 :
					      (new_system - c->system);
			util_idle = (new_idle - c->idle) < 0 ?
					    0 :
					    (new_idle - c->idle);
			util_iowait = (new_iowait - c->iowait) < 0 ?
					      0 :
					      (new_iowait - c->iowait);
			util_irq = (new_irq - c->irq) < 0 ? 0 :
							    (new_irq - c->irq);
			util_softirq = (new_softirq - c->softirq) < 0 ?
					       0 :
					       (new_softirq - c->softirq);
			util_steal = (new_steal - c->steal) < 0 ?
					     0 :
					     (new_steal - c->steal);
			...
			//tuner computations
			busy_use = util_user + util_system + util_nice +
				   util_irq + util_softirq + util_steal;
		...
		enable_direct = direct_greedy_under > 0.99999 ||
				util < direct_greedy_under;
		enable_kick = kick_greedy_under > 0.99999 ||
			      util < kick_greedy_under;

		if (enable_direct)
			cpumask_or(direct_greedy_cpumask, direct_greedy_cpumask,
				   &l->cores);

		if (enable_kick)
			cpumask_or(kick_greedy_cpumask, kick_greedy_cpumask,
				   &l->cores);
	}
}
...
sched_get_core_state(struct ipanema_policy *policy, struct core_event *e)
{
	return per_cpu(core, e->target).state;
}

static void sched_core_entry(struct ipanema_policy *policy,
			     struct core_event *e)
{
...
static void sched_newly_idle(struct ipanema_policy *policy,
			     struct core_event *e)
{
}

static void sched_enter_idle(struct ipanema_policy *policy,
			     struct core_event *e)
{
	struct rusty_core *c = &per_cpu(core, e->target);
...
static void sched_balancing_select(struct ipanema_policy *policy,
				   struct core_event *e)
{
}
...
static s32 try_sync_wakeup(struct task_struct *p, struct rusty_process *rp,
			   s32 prev_cpu)
{
	int cpu;
	struct rusty_core *c;
	...
	share_llc =
		cpumask_test_cpu(prev_cpu, (const struct cpumask *)d_cpumask);
	if (share_llc && test_and_clear_cpu_idle(prev_cpu)) {
		cpu = prev_cpu;
		goto err_out;
	}

	has_idle = cpumask_intersects((const struct cpumask *)d_cpumask,
			    ...
			    int prev_cpu, int wake_flags)
{
	...
	prev_domestic =
		cpumask_test_cpu(prev_cpu, (const struct cpumask *)p_cpumask);
	...
	*/
	...
	if (has_idle_cores) {
		cpu = pick_idle_cpu((const struct cpumask *)p_cpumask,
				    PICK_IDLE_CORE); //TODO
		if (cpu >= 0) {
			goto direct;
		}
    ...
    */
	...
	if (rp->all_cpus && direct_greedy_cpumask &&
		...
		tmp_direct_greedy =
			kzalloc(sizeof(cpumask_size()),
				GFP_KERNEL); //, *tmp_cpumask = NULL;
        ...
        */
		...
		if (has_idle_cores) {
            ...
            */
			...
			if (direct_greedy_cpumask) {
				cpu = pick_idle_cpu((const struct cpumask *)
							    tmp_direct_greedy,
						    PICK_IDLE_CORE);
				if (cpu >= 0) {
					goto direct;
		...
		if (direct_greedy_cpumask) {
			cpu = pick_idle_cpu(
				(const struct cpumask *)tmp_direct_greedy, 0);
...
static void task_pick_domain(struct rusty_process *rp, struct task_struct *p)
{
	...
	for (i = 0; i < nr_doms; i++) {
		dom = (dom + 1) % nr_doms;
		if (cpumask_intersects(p->cpus_ptr,
				       &dom_data[dom]->topology->cores)) {
			rp->dom_mask |= 1LLU << dom;

			cpumask_and(rp->cpumask,
				    (const struct cpumask *)&dom_data[dom]
					    ->topology->cores,
				    p->cpus_ptr);
...
static int sched_new_prepare(struct ipanema_policy *policy,
			     struct process_event *e)
{
    ...
    */
	...
	rp->all_cpus = cpumask_subset((const struct cpumask *)all_cpumask,
				      p->cpus_ptr);
...
*/
static void rusty_enqueue_task(struct rusty_process *rp,
...
*/
static void sched_new_place(struct ipanema_policy *policy,
			    struct process_event *e)
{
...
static void sched_new_end(struct ipanema_policy *policy,
			  struct process_event *e)
{
}

static void rusty_sched_tick(struct ipanema_policy *policy,
			     struct process_event *e)
{
    ...
    */
	...
	if (!rp->slice)
		rusty_enqueue_task(rp, &per_cpu(core, task_cpu(rp->task)).rq,
				   IPANEMA_READY_TICK);
}
...
static int sched_unblock_prepare(struct ipanema_policy *p,
				 struct process_event *e)
{
	return rusty_select_cpu(policy_metadata(e->target), e->target,
				task_cpu(e->target), 0);
}

static void sched_unblock_place(struct ipanema_policy *p,
				struct process_event *e)
{
	struct rusty_process *rp = policy_metadata(e->target);
	int cpu_dst = task_cpu(rp->task);
...
static void sched_unblock_end(struct ipanema_policy *policy,
			      struct process_event *e)
{
}

static void sched_terminate(struct ipanema_policy *policy,
			    struct process_event *e)
{
	struct rusty_process *rp = policy_metadata(e->target);
